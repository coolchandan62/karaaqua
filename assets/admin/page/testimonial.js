...
$(document).on("click", "#authenticateCreateTestimonial", function () {
    if (authenticateCreateTestimonial()) {
      var name = $("input[name='name']").val();
      var designation = $("input[name='designation']").val();
      var image = $("#image").prop("files")[0];
      var content = $("textarea[name='content']").val();
      ...
      $.ajax({
        url: siteUrl + "admin/save-testimonial",
        type: "POST",
        data: formData,
        async: false,
        contentType: false,
  ...
  function authenticateCreateTestimonial() {
    var valid = true;
    var name = $("input[name='name']").val();

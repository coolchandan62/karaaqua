...
class Testimonial extends BaseController{
    ...
    public function createTestimonial(){
        return $this->adminFrontendLoad('testimonial/create-testimonial');
    }
    ...
    public function saveTestimonial(){
        ...
        if ($img = $this->request->getFile('image')) {
            if ($img->isValid() && ! $img->hasMoved()) {
                ...
                $img->move(ROOTPATH . 'assets/uploads/testimonial/', $originalName);
                $data["image"] = $originalName;
            }
        }
        $saveTestimonial = $this->testimonialModel->saveTestimonial($data);
        if($saveTestimonial){
            echo "1";
        }else{
            echo "2";

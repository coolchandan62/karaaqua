...
$(document).on("click", "#authenticateCreateService", function () {
  if (authenticateCreateService()) {
    ...
    $.ajax({
      url: siteUrl + "admin/service/save",
      type: "POST",
      data: formData,
      async: false,
      contentType: false,
      ...
      success: function (result) {
        ...
        if (result.indexOf("1") > -1) {
          ...
          window.location.replace(siteUrl + "admin/service");
...
function authenticateCreateService() {
  var valid = true;
  var heading = $("input[name='heading']").val();
  var heading_slug = $("input[name='heading_slug']").val();
...
$(document).on("click", "#authenticateServiceUpdate", function () {
  if (authenticateServiceUpdate()) {
    ...
    $.ajax({
      url: siteUrl + "admin/service/update",
      type: "POST",
      data: formData,
      async: false,
      contentType: false,
...
function authenticateServiceUpdate() {
  var valid = true;
  var heading = $("input[name='heading']").val();
  var heading_slug = $("input[name='heading_slug']").val();
